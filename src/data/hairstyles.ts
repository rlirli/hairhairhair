...
export const hairstyles: Hairstyle[] = [
  {
    ...
    variations: [
      { id: 'taper-fade-low', name: 'Low taper', description: 'Keeps the transition close to the sideburns and neckline, leaving more of the sides visually intact.' },
      { id: 'taper-fade-higher', name: 'Higher taper / wider fade', description: 'Moves the blend farther around the head; confirm whether your shop uses “higher taper” or “fade” for the requested service.' },
      { id: 'taper-fade-skin', name: 'Skin or bald finish', description: 'Takes the lowest part to skin-level appearance before blending upward.' },
    ],
    consultation: {
      ...
      sampleRequest: '“I’d like a low taper at the temples and neckline, blended but not bald, with the top left long enough to keep its natural shape. Please show me the shortest area before you finish the edges.”',
    },
    considerations: [
  ...
  },
  {
    ...
    variations: [
      { id: 'buzz-cut-even', name: 'Even buzz', description: 'Uses one overall short length for a simple, uniform silhouette.' },
      { id: 'buzz-cut-textured', name: 'Textured buzz', description: 'Leaves enough top length for visible movement or texture while the perimeter stays close.' },
      { id: 'buzz-cut-buzz-fade', name: 'Buzz with taper or fade', description: 'Pairs the short top with a gradually shorter perimeter around the ears, temples, or neckline.' },
    ],
    consultation: {
      ...
      sampleRequest: '“I want a short buzz with a little more length on top, a soft taper around the ears and neck, and a natural edge. Please show me the approximate top length before taking it shorter.”',
    },
    considerations: [
  ...
  },
  {
    ...
    variations: [
      { id: 'twists-individual', name: 'Individual twists', description: 'Separate sections hang away from the scalp, with size and parting determining the visual rhythm.' },
      { id: 'twists-flat', name: 'Flat twists', description: 'Two sections are twisted along the scalp, creating a row-like, scalp-following arrangement.' },
      { id: 'twists-rope', name: 'Rope-style twists', description: 'A tighter or more cord-like finish whose exact technique should be confirmed with the stylist.' },
    ],
    consultation: {
      ...
      sampleRequest: '“I’m looking for medium individual two-strand twists with a visible part pattern, no extensions, and enough length to keep the ends neat. Please keep the tension comfortable and tell me how this differs from a flat twist.”',
    },
    considerations: [
...
export const patternGuidance: PatternGuidance[] = [
  { hairstyleId: 'hairstyle-taper-fade', hairFamilyId: 'hair-family-1', note: 'A taper can keep a straight top clean and graphic; ask how much perimeter contrast you want against the mostly linear fall.' },
  { hairstyleId: 'hairstyle-taper-fade', hairFamilyId: 'hair-family-2', note: 'A taper can preserve visible wave on top; discuss whether the blend should follow the wave’s volume or keep the sides flatter.' },
  { hairstyleId: 'hairstyle-taper-fade', hairFamilyId: 'hair-family-3', note: 'Leave enough top length for the curl shape you want to wear, and ask how the blend will look as curls spring back after cutting.' },
  { hairstyleId: 'hairstyle-taper-fade', hairFamilyId: 'hair-family-4', note: 'The perimeter can be shaped while the coily top stays full or patterned; show the desired height and explain whether the edge should be soft or crisp.' },
  { hairstyleId: 'hairstyle-buzz-cut', hairFamilyId: 'hair-family-1', note: 'A short even cut can read very uniform; discuss scalp visibility, growth direction, and whether a little extra top length is preferred.' },
  { hairstyleId: 'hairstyle-buzz-cut', hairFamilyId: 'hair-family-2', note: 'Wave may flatten or add movement as length changes; ask whether the top should show that movement or be kept close and even.' },
  { hairstyleId: 'hairstyle-buzz-cut', hairFamilyId: 'hair-family-3', note: 'A buzz can reveal curl as texture at longer settings or read more even when shorter; decide by the intended silhouette, not a curl label.' },
  { hairstyleId: 'hairstyle-buzz-cut', hairFamilyId: 'hair-family-4', note: 'Coily texture can change apparent length through shrinkage; ask for a small test area and agree on the final visual length before going shorter.' },
  { hairstyleId: 'hairstyle-twists', hairFamilyId: 'hair-family-1', note: 'Temporary twists may need enough length and a setting plan to hold; discuss section size, direction, and how long the set should last.' },
...
export const styleExamples: StyleExample[] = [
  {
    ...
    lengthDescription: 'Short around the edges and longer through the crown, with the exact length left open for consultation.',
  },
  {
  ...
  },
  {
    ...
    lengthDescription: 'Short and visually even across the top and sides, with the exact guard or setting left to consultation.',
  },
  {
    ...
    lengthDescription: 'The top is visibly longer than the sides; confirm the intended balance and edge finish with the barber.',
  },
  {
    ...
    lengthDescription: 'Shorter sections sit close to the head and can show shrinkage; part size and finished length need a consultation.',
  },
  {
...
export function getHairstylesForFamily(familyId: string): Hairstyle[] {
  const ids = new Set(patternGuidance.filter((guidance) => guidance.hairFamilyId === familyId).map((guidance) => guidance.hairstyleId));
