...
3. Add `PatternGuidance` only for major hair types that the repository material supports. A published guide needs at least one row, but it does not need a row for every type. Absence means the catalog does not currently associate the pair; it does not prove that a person cannot wear the style.
