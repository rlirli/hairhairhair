...
The person page has breadcrumbs, a concise factual mini bio, a source-attributed profile image, a natural profile, and a compact newest-first appearances preview with face-cropped images, capture dates, event titles, and a link to the full `/appearances/` overview. The separate appearances overview retains the detailed source and hairstyle-observation records; its cards link to the dedicated photograph record at `/people/{person}/photographs/{photo}/`. Each photograph record presents the image's creator, source, original file, rights evidence, and every current appearance/guide backlink. The person hairstyle overview at `/people/{person}/hairstyles/` uses explicit editorial ordering and links each hairstyle to `/people/{person}/hairstyles/{hairstyle}/`, where all matching appearance and photograph records are collected. Keep the bio short and factual; it is not a second editorial essay.
