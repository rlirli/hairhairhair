...
> Historical implementation specification. It records the V0 scope and is not the current contributor contract.
...
The `PersonFactSheet` component renders a compact two-column table with short labels and omits traits whose values are `null`. Provenance remains available in the data model but is not rendered as AI, confidence, or editability labels.
